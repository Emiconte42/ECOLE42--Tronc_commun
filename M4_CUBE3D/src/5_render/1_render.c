...
/*   Updated: 2026/07/15 15:27:59 by emiconte         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_pixel(t_game *game, int x, int y, int color)
{
	...
	if (x < 0 || x >= WINDOW_WIDTH || y < 0 || y >= WINDOW_HEIGHT)
		return ;
	pixel = game->img.img_addr + (y * game->img.size_line
			+ x * (game->img.bits_per_pixels / 8));
...
int	ft_init_draw(t_game *game)
{
	...
	game->minimap.tile_size = (int)((double)MINIMAP_MAX_SIZE
			/ (double)game->minimap.size_map);
